...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):
        rgbpath=path+"/rgb"
        p = str(Path(rgbpath).resolve())  # os-agnostic absolute path
        if '*' in p:
    ...
    def __next__(self):
        ...
        # Padded resize
        img = letterbox(img0, self.img_size, stride=self.stride, auto=self.auto)[0]        # Convert
...
class LoadStreams:
    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32, auto=True):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            LOGGER.info(f"{st} Success ({self.frames[i]} framess {w}x{h} at {self.fps[i]:.2f} FPS)")
