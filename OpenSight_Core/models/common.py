...
class DetectMultiBackend(nn.Module):
    ...
    def forward(self, im,cloud, augment=False, visualize=False, val=False):
        # YOLOv5 MultiBackend inference
        b, ch, h, w = im.shape  # batch, channel, height, width
        if self.pt or self.jit:  # PyTorch
            y = self.model(im,cloud) if self.jit else self.model(im,cloud, augment=augment, visualize=visualize)
